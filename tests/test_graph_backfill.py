...
def test_graph_backfill_enqueues_existing_entities_in_dependency_order(
    ...
    assert (result.cards, result.topics, result.notes, result.problems, result.total) == (
        1,
        1,
        1,
        1,
    ...
    assert "content_markdown" not in events[2].payload["entity"]
    assert "answer" not in events[3].payload["entity"]
