...
랜덤 학습에서는 문제 묶음의 답안을 모두 제출한 뒤 한 화면에서 일괄 채점합니다. 객관식과 O/X는 선택한 답과 등록된 정답을 자동으로 비교하고, 단답형·주관식·빈칸 추론은 전체 채점 화면에서 사용자가 기준 답안·해설을 확인한 뒤 직접 정답 또는 오답으로 판정합니다. 문제별 답안 내용은 저장하지 않지만 출제·정답·오답 횟수는 DB에 누적합니다.

랜덤 조회는 기본적으로 한 문제를 반환합니다. `limit` 쿼리 파라미터로 최대 100개까지 가중치 기반으로 조회할 수 있습니다. 가중치는 아래 식으로 계산하며, 오답 비율이 높거나 출제 횟수가 적은 문제일수록 선택 확률이 높아집니다.
